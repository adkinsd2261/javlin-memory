...
class GitHubSyncer:
    ...
    def run_auto_sync(self, force: bool = False) -> Dict[str, any]:
        ...
        if not force and not self.config['git_sync']['auto_push']:
            return {"status": "disabled", "message": "Auto-push is disabled in config"}
        ...
        if not force and not self.should_auto_sync(recent_logs):
            return {"status": "no_trigger", "message": "No significant changes to warrant auto-sync"}
